#
# iOS side of the Wise Signals Flutter plugin. Standard Flutter federated-plugin
# podspec. Run `flutter pub get` then build the host app to validate.
#
Pod::Spec.new do |s|
  s.name             = "wise_signals"
  s.version          = "0.1.0"
  s.summary          = "Wise Signals — Flutter SDK (iOS bridge)."
  s.description      = "Thin Flutter bridge over the native WiseFingerprint iOS SDK."
  ...
  # M6/M7). Until a CocoaPods coordinate exists, add the `WiseFingerprint` Swift
  # Package to the host app (Xcode → Package Dependencies); it links into the same
  # binary so `import WiseFingerprint` in WiseSignalsPlugin.swift resolves. Once
  # M6 publishes a pod, uncomment + pin the real coordinate (brand name bakes in):
  #
  #   s.dependency "WiseFingerprint", "~> 0.1"
