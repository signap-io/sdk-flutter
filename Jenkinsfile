...
// (flutter pub publish --dry-run) on PRs. The pub.dev token comes from Secrets Manager via
// the flutter-builder agent's IRSA role — see infra/runbooks/npm-pubdev-publish.md §4.
//
// ⚠️ NOT seeded yet (commented in infra/jenkins/seed_job.groovy): (1) the flutter-builder agent
// image must carry the Flutter/Dart SDK + awscli, (2) the jenkins-flutter-builder IRSA role
// needs GetSecretValue on signap/pub/*, (3) the pub.dev verified publisher on signap.io (or a
// PUB_TOKEN) must exist. Enable the seed entry once all three are verified (runbook §4).
